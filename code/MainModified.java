...
public class MainModified {
    public static void main(String[] args){
        ...
        // Low bounded list is created to obtain greenish colors
        int[] randomColor = new int[6];
        int[] randomColorLowBounded = new int[12];

        for(int i = 0 ; i < 6 ; i++ ){
            Random random = new Random();
            randomColor[i]= random.nextInt(32,256);
        }

        for(int i = 0 ; i < 12; i++ ){
            Random random = new Random();
            randomColorLowBounded[i]= random.nextInt(0,16);
        }
        // Color array for bricks (first import java.awt.Color )
        Color[] colors = { new Color(randomColorLowBounded[0],randomColor[0] ,randomColorLowBounded[1]),
                new Color(randomColorLowBounded[2], randomColor[1],randomColorLowBounded[3]),
                new Color(randomColorLowBounded[4],  randomColor[2], randomColorLowBounded[5]),
                new Color(randomColorLowBounded[6], randomColor[3], randomColorLowBounded[7]),
                new Color(randomColorLowBounded[8], randomColor[4],randomColorLowBounded[9]),
                new Color(randomColorLowBounded[10],  randomColor[5], randomColorLowBounded[11])
        };
        ...
        // Position and order of superpowers
        double circleRadius = 25;
        double[][] circles = {
                {150, 100}, // Eliminating random amount of bricks (1 to 5) bricks
                {650, 100}, // Paddle width * 2
                {400, 80}, // Paddle width + 5
                {250, 130}, // Ball radius * 2
                {350, 60}, // Ball radius + 5
                {100, 40} // Score for a brick * 2
        };

        // Selecting random superpowers
        int[] randomNumbers = new int[2];
        Random random = new Random();
        randomNumbers[0]= random.nextInt(0,5);
        randomNumbers[1]= random.nextInt(0,5);

        // Avoiding the two random numbers are the same
        ...
        // Controlling whether the superpower is used
        boolean[] usedEffects = new boolean[6];

        //STRINGS
        // The texts for superpowers to identify them
        String[] powerUpTexts = {
                "Eliminate 1-5 Bricks",
                "Paddle Width x2",
                "Paddle Width +5",
                "Ball Radius x2",
                "Ball Radius +5",
        ...
        while (!isGameOver && !isVictory){
            ...
            if (isPaused){
                ...
                for (int i = 0; i < randomNumbers.length; i++) {
                    ...
                    // If the effect is used, re-execution of it is prevented.
                    if (usedEffects[currentNumber]) {
                        continue;
                    }

                    // Assigning coordinates and colors to each superpower.
                    if (currentNumber == 0) {
                        StdDraw.setPenColor(StdDraw.GREEN);
                        StdDraw.filledCircle(circles[0][0], circles[0][1], circleRadius);
                    } else if (currentNumber == 1) {
                        StdDraw.setPenColor(paddleColor);
                        StdDraw.filledCircle(circles[1][0], circles[1][1], circleRadius);
                    } else if (currentNumber == 2) {
                        StdDraw.setPenColor(paddleColor);
                        StdDraw.filledCircle(circles[2][0], circles[2][1], circleRadius);
                    } else if (currentNumber == 3) {
                        StdDraw.setPenColor(ballColor);
                        StdDraw.filledCircle(circles[3][0], circles[3][1], circleRadius);
                    } else if (currentNumber == 4) {
                        StdDraw.setPenColor(ballColor);
                        StdDraw.filledCircle(circles[4][0], circles[4][1], circleRadius);
                    } else if (currentNumber == 5) {
                        StdDraw.setPenColor(StdDraw.BLACK);
                        StdDraw.filledCircle(circles[5][0], circles[5][1], circleRadius);
                    }
                    // If the superpower is not used draw it to the canvas.
                    if (!usedEffects[currentNumber]) {
                        StdDraw.setPenColor(StdDraw.BLACK);
                        StdDraw.text(circles[currentNumber][0], circles[currentNumber][1] + circleRadius + 10, powerUpTexts[currentNumber]);
                    }
                }
                ...
                // Placing superpowers to the canvas
                // The superpowers have its own color (e.g the brick superpower is green since bricks are greenish)
                ...
                for (int i = 0; i < randomNumbers.length; i++) {
                    ...
                    // If the effect is not used and the ball went inside the superpower circle, the effect activates.
                    if (!usedEffects[currentNumber] && Math.sqrt(Math.pow(newBallPos[0] - circles[currentNumber][0], 2)
                            + Math.pow(newBallPos[1] - circles[currentNumber][1], 2)) < ballRadius + circleRadius) {

                        // Activating the features.
                        // Removing random amount of random bricks
                        if (currentNumber == 0) {
                            int bricksToRemove = random.nextInt(0, 5);
                            int removed = 0;

                            while (removed < bricksToRemove) {
                                int index = random.nextInt(0, brickHitStatus.length);
                                if (!brickHitStatus[index]) {
                                    brickHitStatus[index] = true;
                                    removed++;
                                }
                            }
                        }
                        ...
                        usedEffects[currentNumber] = true;
                    }
                }
